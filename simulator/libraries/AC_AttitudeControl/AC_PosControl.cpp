...
void AC_PosControl::run_xy_controller(float dt)
{
	...
	_attitude_control.setControlXY(_accel_target.x, _accel_target.y,
                            ...
                            _vel_error.x, _vel_error.y, flag);
	/*
	...
	*/
    ...
    /*
     * Pritam Dash
     * Writing Position Controller data to File
     */
    /*Write_Target_posX(_pos_target.x);
    Write_Target_posX(ekfNavVelGainScaler);
    Write_Target_posX(curr_pos.x);
    Write_Target_posX(curr_pos.y);
    Write_Target_posX(curr_pos.z);
    Write_Target_posX(kP);
    Write_Target_posX(_pos_error.x);
    Write_Target_posX(_vel_target.x);
    Write_Target_posX(_vel_target.y);
    Write_Target_posX(_vel_target.z);
    Write_Target_posX(_vehicle_horiz_vel.x);
    Write_Target_posX(_vehicle_horiz_vel.y);
    Write_Target_posX(_inav.get_velocity().z);
    Write_Target_posX(_vel_error.x);
    Write_Target_posX(_vel_error.y);
    Write_Target_posX(vel_xy_p.x);
    Write_Target_posX(vel_xy_i.x);
    Write_Target_posX(vel_xy_d.x);
    Write_Target_posX(_accel_target.x);
    Write_Target_posX(_accel_target.y);
    Write_Target_posX(_vehicle_horiz_vel.x);
    Write_Target_posX(_roll_target);
    Write_PosX();*/

    /*Write_Target_posY(_pos_target.y);
    Write_Target_posY(ekfNavVelGainScaler);
    Write_Target_posY(kP);
    Write_Target_posY(_vel_target.y);
    Write_Target_posY(vel_xy_p.y);
    Write_Target_posY(vel_xy_i.y);
    Write_Target_posY(vel_xy_d.y);
    Write_Target_posY(_accel_target.y);
    Write_Target_posY(_vehicle_horiz_vel.y);
    Write_Target_posY(_pitch_target);
    Write_PosY();*/
...
/*void AC_PosControl::Write_Pos(float data)
{
    MyClass::write_to_file(data);
}

void AC_PosControl::Write_Pos()
{
    MyClass::write_to_file();
}*/

/*void AC_PosControl::Write_PosX()
{
    MyPIDio::write_to_file_POSX();
}

void AC_PosControl::Write_Target_posX(float data)
{
    MyPIDio::write_to_file_Target_posX(data);
}*/

/*void AC_PosControl::Write_PosY()
{
    MyPIDio::write_to_file_POSY();
}

void AC_PosControl::Write_Target_posY(float data)
{
    MyPIDio::write_to_file_Target_posY(data);
}*/
